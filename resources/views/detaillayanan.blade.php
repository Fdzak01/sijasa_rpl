...
<body>
    ...
    <!-- Main -->
    <section class="relative max-w-4xl mx-auto mt-12 p-6">
        <!-- Background Image -->
        <div class="relative">
            <img src="{{ asset('storage/' . $data->thumbnail) }}') }}" alt="Background Image"
                class="w-full h-48 object-cover rounded-lg" />
            <!-- Floating Card -->
            <div class="absolute top-1/2 left-6 transform -translate-y-1/2 bg-white shadow-lg rounded-lg p-6 w-2/3">
                <h2 class="text-xl font-bold text-gray-800">
                    {{ $data->service }}
                </h2>
                <div class="flex items-center justify-between mt-4">
                    <span class="text-lg font-semibold text-teal-600">Rp. {{ $data->price }}</span>
                    <a href="#" class="text-teal-600 hover:underline">{{ $data->kategori_layanans->kategori }}</a>
                </div>
                <small>{{ $data->estimate }}</small>
            </div>
        </div>
        ...
        <!-- Pilih Paket Card -->
        <div class="bg-teal-600 text-white rounded-lg p-4 mt-6 text-center">
            ...
            <form action="{{ route('order.cart') }}" method="POST">
                ...
                <button class="mt-4 px-6 py-2 bg-white text-teal-600 rounded-full hover:bg-gray-100">
                    Pilih
                </button>
            </form>
        </div>
