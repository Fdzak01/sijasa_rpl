...
<body>
    ...
    <!-- Main -->
    <section class="relative min-h-screen min-w-7xl">
        <!-- Background main -->
        <img src="{{ asset('assets/users/img/Layanan Tukang Bersih Bersih Rumah dan Bangunan Komersial 1.jpg') }}"
        ...
        <div class="relative flex flex-col justify-center min-h-screen z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-2 gap-12">
                <div class="bg-black bg-opacity-50 p-8 rounded-box items-center text-center space-y-6">
                    <h1 class="text-4xl font-bold text-white">Metode Pembayaran</h1>
                    <!-- Bank -->
                    ...
                    <form action="{{ route('payment.order') }}" method="POST">
                        ...
                        <button type="submit">
                            <div class="bg-white bg-opacity-75 p-4 rounded-box">
                                <div class="flex items-center gap-12">
                                    <svg width="69" height="73" viewBox="0 0 69 73" fill="none"
                                        xmlns="http://www.w3.org/2000/svg" class="h-auto">
                                        <path
                                            d="M11.5 45.6248V37.5076M26.8334 45.6248V37.5076M42.1667 45.6248V37.5076M57.5 45.6248V37.5076M6.90002 55.9665H62.1V65.6998H6.90002V55.9665ZM6.90002 26.7665V20.2776L33.3658 7.2998L62.1 20.2776V26.7665H6.90002Z"
                                            stroke="#008080" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" />
                                    </svg>
                                    <span class="text-4xl font-medium text-teal-600">Bank</span>
                                </div>
                            </div>
                        </button>
                    </form>
                    ...
                    <form action="{{ route('payment.order') }}" method="POST">
                        ...
                        <button type="submit">
                            <div class="bg-white bg-opacity-75 p-4 rounded-box">
                                <div class="flex items-center gap-12">
                                    <svg width="74" height="77" viewBox="0 0 74 77" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M54.76 9.75293H12.0045C9.4615 9.75293 7.40551 12.5976 7.40314 15.5279C7.40002 19.3779 9.4615 23.1852 12.0045 23.1852H60.0222C63.655 23.1852 66.6 22.3995 66.6 26.1796V59.0329C66.6 63.569 63.0661 67.2463 58.7067 67.2463H15.2934C10.934 67.2463 7.40002 63.569 7.40002 59.0329V17.4529M50.8649 42.5527L50.8134 42.6063"
                                            stroke="#008080" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" />
                                    </svg>

                                    <span class="text-4xl font-medium text-teal-600">E-wallet</span>
                                </div>
                            </div>
                        </button>
                    </form>
                            ...
                            <div class="bg-white bg-opacity-75 p-4 rounded-box">
                                <div class="flex items-center gap-12">
                                    <svg width="59" height="73" viewBox="0 0 59 73" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M21.4971 40.3089C21.884 40.3089 30.8594 40.7124 36.1983 39.3406H36.237C42.3883 37.7669 50.9382 33.2478 53.1434 18.48C53.1434 18.48 58.0566 0 33.6836 0H12.7538C10.8581 0 9.2332 1.45257 8.9237 3.38934L0.0256149 62.1379C-0.167822 63.3484 0.760674 64.4782 1.92129 64.4782H15.1911L18.4408 43.0123C18.6729 41.4791 19.9496 40.3089 21.4971 40.3089Z"
                                            fill="#008080" />
                                        <path
                                            d="M56.5479 21.6807C53.4142 36.7309 43.5489 44.6798 27.8419 44.6798H22.1548L18.17 70.9875C18.0153 72.0366 18.789 72.9646 19.7949 72.9646H27.1455C28.4609 72.9646 29.6215 71.9559 29.8149 70.584C30.1244 68.97 31.8267 57.188 32.1749 55.1706C32.3683 53.7987 33.5289 52.79 34.8443 52.79H36.5465C47.4564 52.79 56.0063 48.1498 58.5209 34.7942C59.5268 29.3874 58.9852 24.949 56.5479 21.6807Z"
                                            fill="#008080" />
                                    </svg>

                                    <span class="text-4xl font-medium text-teal-600">PayPal</span>
                                </div>
                            </div>
                        </button>
                    </form>
                ...
                <div class="bg-black bg-opacity-50 p-8 rounded-box items-center text-center space-y-6">
                    <h1 class="text-4xl font-bold text-white">Detail Pembayaran</h1>
                    <!-- Bank -->
                    @foreach ($jasa as $item)
                        <div class="bg-white bg-opacity-75 p-4 rounded-box">
                            <h1 class="text-3xl font-medium text-teal-600">Layanan</h1>
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-lg text-teal-600">{{ $item->service }}</span>
                                <span class="text-lg text-teal-600">Rp. {{ $item->price }}</span>
                            </div>
                            <span class="text-sm text-gray-500 block mb-4">{{ $item->service }}</span>

                            @foreach ($data as $cart)
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-lg text-teal-600">Biaya Layanan</span>
                                    <span class="text-lg text-teal-600">Rp. {{ $cart->biaya_pelayanan }}</span>
                                </div>
                                <div class="flex justify-between items-center mb-4">
                                    <span class="text-lg text-teal-600">Admin</span>
                                    <span class="text-lg text-teal-600">Rp. {{ $cart->biaya_admin }}</span>
                                </div>
                            @endforeach

                            <div class="flex justify-between items-center font-bold text-teal-800 text-xl">
                                <span>Total</span>
                                <span>Rp. {{ $item->price + $cart->biaya_pelayanan + $cart->biaya_admin }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
